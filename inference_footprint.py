...
def parse_args():
    ...
    if not os.path.exists(args.model):
        raise ValueError('Inference model not found')
...
class E2PoseInference_by_trt(E2PoseInference):
    ...
    def __call__(self, x, training=False):
        output = self.predict(x)
        humans = self.decode(output, src_hw=[x.shape[2], x.shape[3]], th=0.3)
...
def draw_footprints(image, foot_data_list, current_frame, frame_rate=30):
    ...
    # If the frame number of the foot data is less than or equal to the current frame number and greater than the current frame number minus frame_rate...
        if foot_data['Frame'] <= current_frame and foot_data['Frame'] > current_frame - frame_rate/8:
            ...
            cv2.circle(image, (mid_point_x, mid_point_y), radius=10, color=(255, 0, 0), thickness=-1)

    return image


def calculate_frame_time(filename, frame_no):
    # 動画ファイルを読み込む
    ...
    # フレーム番号と時間を返す
    return frame_no, time_in_seconds
...
if __name__ == "__main__":
    ...
    with draw.seaquence_writer(args.dst, dev=args.dev) as writer:
        ...
        for ii, (src_path, raw, frame) in tqdm.tqdm(enumerate(draw.read_src(args.src, resize=model.inputs[0].shape[1:3]))):
            if first_frame:
                ...
                first_frame = False
            # Initialize another video writer
            if ii == 0:
                video_filename = os.path.basename(src_path)
                base_name = os.path.splitext(video_filename)[0]
                footprint_path = args.dst / 'footprint' / f'{base_name}_footprint.mp4'
                fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
                footprint_writer = cv2.VideoWriter(str(footprint_path), fourcc, 30.0, (raw.shape[1], raw.shape[0]))
            ...
            # Create an image of the court
            court_img = np.zeros_like(raw)
            court_img = draw_court_on_frame(court_img, court_coords)

            # Create a footprint image
            footprints = np.zeros_like(raw)
            footprints = draw_footprints(footprints, foot_data_list, ii, frame_rate=30)

            # Combine court image and footprints
            combined = cv2.addWeighted(court_img, 0.3, footprints, 0.7, 0)

            # Output the footprint image to the new video file
            footprint_writer.write(combined)
            ...
            # 動画ファイルを読み込む
            clip = VideoFileClip(f'./Inputs/{video_filename}')
            # 動画のフレームレート（fps）を取得
            fps = clip.fps
            ...
            for human in humans:
                ...
                # Calculate frame number and time
                frame_no, time_in_seconds = calculate_frame_time(f'./Inputs/{video_filename}', ii)
    ...
    for src in args.src:
        ...
        csv_path = pathlib.Path(f'{args.dst}/csv/data_foot_{base_name}.csv')
                ...
                # Add court_coords to foot_data_list
        for foot_data in foot_data_list:
            foot_data['Court_Coords'] = court_coords

        # Write the contents of foot_data_list to a CSV file
        with open(csv_path, 'w', newline='') as csvfile:
            fieldnames = ['Frame','Time', 'LeftFoot_X', 'LeftFoot_Y', 'RightFoot_X', 'RightFoot_Y', 'Court_Coords']
